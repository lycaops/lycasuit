...
import { BarChart3, FileSignature, Receipt, LifeBuoy, LayoutGrid, Users, Home, KeyRound } from "lucide-react"
...
function HomeSidebar({
  ...
  return (
    <aside className="hidden w-72 shrink-0 flex-col overflow-hidden bg-[#21264e] px-6 py-6 text-white lg:flex lg:h-dvh lg:max-h-dvh lg:sticky lg:top-0">
        ...
        {tools.map((tool) => {
          ...
          return (
            <Link key={tool.key} href={tool.route} className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-white/65 transition hover:bg-white/10 hover:text-white">
              ...
              {TOOL_LABELS[tool.key] ? t(TOOL_LABELS[tool.key]) : tool.name}
            </Link>
          )
        })}
      </nav>
